...
class Staff extends User {
  ...
  getLatestMessage(authId) {
    ...
    return latestMessage ? latestMessage.message : "No message";
  }
  ...
  getLatestDate(authId) {
    ...
    let dayDifference = dateUtility.getDateDifferenceInDays(
      dateToday,
      latestMessageDate
    );
    if (!latestMessage) return "-";
    else if (dayDifference < 1)
      return dateUtility.getFormattedTime(latestMessageDate);
    else if (dayDifference < 7)
      return dateUtility.getFormattedDayNow(latestMessageDate);
